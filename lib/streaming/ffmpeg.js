...
ffmpeg.spawnChild = function (args) {
...
ffmpeg.getEncoder = function (options, rotation) {
    ...
    case 'mp4-mse':
        vf += ",fps=fps=25";
        args.push('-vf', vf);
        args.push('-c:v', 'h264', '-preset', 'ultrafast', '-maxrate', '500k', '-bufsize', '2M', '-crf', 23, '-threads', ffmpeg.threads, '-g', 25);
        args.push('-f', 'mp4', '-reset_timestamps', 1, '-movflags', '+empty_moov+frag_keyframe+default_base_moof', '-pix_fmt', 'yuv420p', '-r', 25, '-');
        break;
    ...
    var child = ffmpeg.spawnChild(args);
...
ffmpeg.streamRTSP = function (camera) {
    ...
    var args = ['-loglevel', ffmpeg.loglevel, '-f', 'rtsp', '-rtsp_transport', 'tcp', '-i', url, '-vcodec', 'copy',
        '-f', 'mp4', '-movflags', '+empty_moov+frag_keyframe+default_base_moof', '-reset_timestamps', 1];
    ...
    var output = child.stdout.pipe(segmenter);
